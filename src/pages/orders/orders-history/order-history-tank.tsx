...
import OrderHistoryTankList from "./order-history-tank-list";
...
const OrderHistoryTank = () => {
  const dispatch = useDispatch();
  const history = useHistory();
  const state = history?.location?.state as { page: string };

  const historyOrders = useSelector((state: TRootState) => state.orders.orderTankData);
  ...
  return (
    <OrdersHistory>
      <div className="tab-pane fadeIn active" id="tab-2">
        ...
        <Pagination
          ItemsComponent={OrderHistoryTankList}
...
export default OrderHistoryTank;

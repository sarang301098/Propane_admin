...
import PendingOrdersFuelList from "./pending-orders-fuel-list";
...
const PendingOrdersFuel = () => {
  const dispatch = useDispatch();
  const history = useHistory();
  const state = history?.location?.state as { page: string };

  const pendingOrders = useSelector((state: TRootState) => state.orders.orderFuelData);
  ...
  return (
    <PendingOrders>
      <div className="tab-pane fadeIn active" id="tab-1">
        ...
        <Pagination
          ItemsComponent={PendingOrdersFuelList}
...
export default PendingOrdersFuel;

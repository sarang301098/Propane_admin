...
import AllOrdersFuelList from "./all-orders-fuel-list";
...
const AllOrdersFuel = () => {
  ...
  const allOrders = useSelector((state: TRootState) => state.orders.orderFuelData);
  ...
  const [startDate, setStartDate] = useState<moment.Moment | null | undefined>(
    state?.todayOrders ? moment().startOf("day") : null
  );
  const [endDate, setEndDate] = useState<moment.Moment | null | undefined>(
    state?.todayOrders ? moment().endOf("day") : null
  );
  ...
  const fetchAllOrders = (pages?: number) => {
    dispatch(
      getOrdersFuelActionThunk(
        ...
        orderStatus === "All"
          ? ["pending", "ongoing", "refilled", "rescheduled", "delivered", "cancelled", "cancelled_by_admin"]
    ...
    <AllOrders>
      <div className="tab-pane fadeIn active" id="tab-1">
        <div className="p-3 d-flex justify-content-end inner-filter">
          <div className="m-l-10">
            <div className="input-group w-250">
              ...
              <div className="input-group-append">
                <button type="button" className="input-group-text pointer" onClick={() => fetchAllOrders()}>
          ...
          <div className="m-l-10">
            <button onClick={() => fetchAllOrders()} type="button" className="btn btn-dark">
              Submit
            </button>
          </div>
          <div className="m-l-10">
        ...
        <Pagination
          ItemsComponent={AllOrdersFuelList}
          pageCount={allOrders ? allOrders.count : 1}

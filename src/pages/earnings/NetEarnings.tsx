...
import NetEarningsList from "./NetEarningsList";
import Pagination from "../../components/pagination/Pagination";
import TRootState from "../../store/root.types";
import { getEarningActionThunk } from "../../store/earnings/earning.actions.async";
...
const NetEarnings: React.FC = () => {
  const dispatch = useDispatch();
  const history = useHistory();
  const state = history?.location?.state as { page: string };

  const netEarningCount = useSelector(
    (state: TRootState) => state?.earning?.netEarningList?.count
  );
  ...
  /**
   * fetch net orders earning
   * @param pages
   */
  const fetchEarnings = (pages?: number) => {
    dispatch(
      getEarningActionThunk(
        pages || page,
        itemsPerPage,
              ...
              <div className="card card-tabs clearfix">
                ...
                <div className="card-body p-0">
                  <div className="tab-content">
                    <Pagination
                      ItemsComponent={NetEarningsList}
                      dispatchAction={fetchEarnings}
                      pageCount={netEarningCount}
                      page={page}
                      setPage={setPage}
                    />
                  </div>
